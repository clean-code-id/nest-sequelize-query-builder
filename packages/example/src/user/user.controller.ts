...
import {
  ...
  QueryBuilderParams,
  AllowedSort,
} from "@cleancode-id/nestjs-sequelize-query-builder";
...
export class UserController {
   ...
   *   GET /users?sort=name&page=1&size=3
   *   GET /users?sort=-postCount&page=2&size=3
   */
  @Get()
  async index(@QueryBuilderParams() params: any) {
    try {
      ...
      if (params.page) {
        ...
        const perPage = params.size ? parseInt(params.size) : 10;
